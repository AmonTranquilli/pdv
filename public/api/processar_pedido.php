...
try {
    ...
    $stmt_pedido = $conn->prepare(
        "INSERT INTO pedidos (id_cliente, nome_cliente, telefone_cliente, endereco_entrega, numero_entrega, bairro_entrega, complemento_entrega, referencia_entrega, data_pedido, total_pedido, forma_pagamento, troco_para, troco, observacoes_pedido, status) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?, ?, 'pendente')"
    );
    $id_cliente = $cliente_data['id'] ?? null;
    $stmt_pedido->bind_param("isssssssdssss",
        $id_cliente, $cliente_data['nome'], $cliente_data['telefone'], $cliente_data['endereco'],
        $cliente_data['numero'], $cliente_data['bairro'], $cliente_data['complemento'],
        $cliente_data['ponto_referencia'], $total_pedido, $forma_pagamento,
        $troco_para, $troco, $observacoes_pedido
    );
    ...
    foreach ($carrinho as $item) {
        $stmt_item->bind_param("iisids", 
            $pedido_id, $item['id'], $item['nome'], $item['quantidade'], 
            $item['preco_unitario'], $item['obs']
        );
    ...
    // --- CÓDIGO DO BOT (VERSÃO SIMPLIFICADA) ---
