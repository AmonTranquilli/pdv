...
// Pega as datas do formulário se ele foi enviado, senão, usa o padrão
$data_inicio = $_POST['data_inicio'] ?? $data_inicio_padrao;
$data_fim = $_POST['data_fim'] ?? $data_fim_padrao;

// Processa o cálculo somente se o formulário for enviado
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $valor_diaria = (float)$funcionario['valor_diaria'];

        // a. Buscar dias de funcionamento da loja
        ...
        // b. Contar dias úteis no período selecionado
        ...
        // c. Contar faltas injustificadas no período
        ...
        // d. Somar todos os vales PENDENTES
        $stmt_vales = $conn->prepare("SELECT id, valor FROM vales_funcionarios WHERE id_funcionario = ? AND id_pagamento_descontado IS NULL");
        $stmt_vales->bind_param("i", $funcionario_id);
        $stmt_vales->execute();
        $vales_pendentes = $stmt_vales->get_result()->fetch_all(MYSQLI_ASSOC);
        $stmt_vales->close();

        $total_vales = array_sum(array_column($vales_pendentes, 'valor'));
        $ids_vales_descontados = array_column($vales_pendentes, 'id');

        // e. Calcular o pagamento final de forma detalhada
        ...
        $calculo_detalhes = [
            ...
            'ids_vales_usados' => $ids_vales_descontados
        ];
    } catch (Exception $e) {
...
<div class="payment-layout">
    <div class="card-style">
        ...
        <form method="POST" action="gerar_pagamento.php?id=<?= $funcionario_id ?>">
            <div class="form-group">
                <label for="data_inicio">Período - Data de Início:</label>
                <input type="date" id="data_inicio" name="data_inicio" value="<?= htmlspecialchars($data_inicio) ?>" required>
            </div>
            <div class="form-group">
                <label for="data_fim">Período - Data de Fim:</label>
    ...
    <div class="card-style">
        ...
        <?php if ($calculo_detalhes): ?>
            <div class="calculation-result">
                <div class="result-line"><span>Total de dias de funcionamento:</span><strong><?= $calculo_detalhes['dias_uteis'] ?></strong></div>
                <div class="result-line"><span>Faltas:</span><strong><?= $calculo_detalhes['faltas'] ?></strong></div>
                <div class="result-line"><span>Total de diárias a pagar:</span><strong><?= $calculo_detalhes['dias_pagos'] ?></strong></div>
                <hr style="border: none; border-top: 1px solid #eee; margin: 10px 0;">
                <div class="result-line"><span>Salário Bruto (Dias de Funcionamento x valor):</span><strong>R$ <?= number_format($calculo_detalhes['salario_bruto'], 2, ',', '.') ?></strong></div>
                <div class="result-line desconto"><span>(-) Faltas no período:</span><strong>- R$ <?= number_format($calculo_detalhes['desconto_faltas'], 2, ',', '.') ?></strong></div>
                <div class="result-line desconto"><span>(-) Vales / Adiantamentos:</span><strong>- R$ <?= number_format($calculo_detalhes['total_vales'], 2, ',', '.') ?></strong></div>
                <div class="result-line total"><span>Valor Final a Pagar:</span><strong>R$ <?= number_format($calculo_detalhes['valor_final'], 2, ',', '.') ?></strong></div>
                <button id="btnConfirmarPagamento" class="btn-register-payment">Confirmar e Registrar Pagamento</button>
            </div>
        <?php else: ?>
            <p class="placeholder-text">Selecione um período e clique em "Calcular" para ver o resumo do pagamento aqui.</p>
        <?php endif; ?>
    </div>
</div>
        ...
        if (btnConfirmar) {
            btnConfirmar.addEventListener('click', async () => {
                ...
                const dadosPagamento = {
                    ...
                    ids_vales_descontados: <?= json_encode($calculo_detalhes['ids_vales_usados'] ?? []) ?>
                };

                try {
                    const response = await fetch('registrar_pagamento_completo.php', {
                        method: 'POST',
                        headers: {
                    ...
                    if (result.sucesso) {
                        window.location.href = 'historico_pagamentos.php';
                    }
                } catch (error) {
